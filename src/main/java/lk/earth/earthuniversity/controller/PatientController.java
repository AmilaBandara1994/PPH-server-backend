...
//    @PreAuthorize("hasAuthority('employee-select')")
    public List<Patient> get(@RequestParam HashMap<String, String> params) {
        ...
        if (patientstatusid != null)
            patientStream = patientStream.filter(e -> e.getPatientstatus().getId() == Integer.parseInt(patientstatusid));
        if (genderid != null)
            patientStream = patientStream.filter(e -> e.getGender().getId() == Integer.parseInt(genderid));
        if (contactnumber != null)
            patientStream = patientStream.filter(e -> e.getContactnumber().equals(contactnumber));
        ...
        if (bloodgroupid != null)
            patientStream = patientStream.filter(e -> e.getBloodgroup().getId() == Integer.parseInt(bloodgroupid));
